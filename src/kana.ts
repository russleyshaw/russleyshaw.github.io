...
const HIRI_SUKUON_CHAR = "っ";
const KATA_SUKUON_CHAR = "ッ";
...
export function isSukuon(char: string): boolean {
    return char === "っ" || char === "ッ";
}

export function hasDakuten(char: string): boolean {
    return char in DAKUTEN_MAP;
}

export function hasHandakuten(char: string): boolean {
    return char in HANDAKUTEN_MAP;
}

export function addDakuten(char: string): string {
    if (char in DAKUTENABLE_MAP) {
        return char + DAKUTEN_CHAR;
    }
